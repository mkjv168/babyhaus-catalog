...
export function CartDrawer() {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex justify-end">
      ...
      {/* Drawer */}
      <div className="relative w-full max-w-md bg-[#faf8f5] h-full flex flex-col shadow-2xl animate-in slide-in-from-right duration-200">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#e8e4df] bg-white">
          <div>
            <h2 className="text-lg font-bold text-[#2d2d2d]">Your Cart</h2>
            <p className="text-xs text-[#7a7a7a]">{totalItems} item{totalItems !== 1 ? 's' : ''}</p>
          </div>
          <button
            onClick={() => setIsOpen(false)}
            className="w-9 h-9 flex items-center justify-center rounded-full bg-[#f5f1ec] text-[#7a7a7a] hover:bg-[#e8e4df] transition-colors"
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {items.length === 0 ? (
            <div className="text-center py-16 text-[#7a7a7a]">
              ...
              <button
                onClick={() => setIsOpen(false)}
                className="mt-4 px-5 py-2 bg-[#d4a574] text-white text-sm font-semibold rounded-full hover:bg-[#c49464] transition-colors"
              >
                Continue Shopping
              </button>
            </div>
            ...
            items.map((item) => (
              <div key={item.id} className="flex gap-3 bg-white rounded-xl border border-[#e8e4df] p-3">
                <Link href={`/product/${item.id}`} onClick={() => setIsOpen(false)} className="flex-shrink-0">
                  <div className="w-16 h-16 rounded-lg bg-[#f5f1ec] overflow-hidden">
                ...
                <div className="flex-1 min-w-0">
                  <Link href={`/product/${item.id}`} onClick={() => setIsOpen(false)}>
                    <h3 className="text-sm font-semibold text-[#2d2d2d] line-clamp-2 leading-snug hover:text-[#d4a574] transition-colors">
                      {item.name}
                    </h3>
                  </Link>
                  <p className="text-xs text-[#7a7a7a] mt-0.5">{item.brand || item.category}</p>
                  <div className="flex items-center justify-between mt-2">
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        className="w-7 h-7 flex items-center justify-center rounded-full bg-[#f5f1ec] text-[#2d2d2d] text-sm font-bold hover:bg-[#e8e4df] transition-colors"
                      >
                        -
                      </button>
                ...
                <button
                  ...
                  className="flex-shrink-0 w-7 h-7 flex items-center justify-center rounded-full text-[#b0aba5] hover:text-red-500 hover:bg-red-50 transition-colors self-start"
        ...
        {/* Footer */}
        {items.length > 0 && (
          <div className="border-t border-[#e8e4df] bg-white p-4 space-y-3">
